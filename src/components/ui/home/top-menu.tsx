    ...
    return (
        <div className="bg-primary w-full text-white">
            ...
            <div className="md:hidden px-4 sm:px-6 py-2">
                <div className="flex gap-1 items-center mb-2"><MapPin size={16} className="hidden sm:block" /> <small>{COMPANY_ADDRESS}</small></div>
                <ul className="flex justify-between gap-2">
                    <span>
                        <li className="flex gap-1 items-center text-xs"><Phone size={14} /><a href={`tel:${PRIMARY_PHONE_NUMBER}`}>{PRIMARY_PHONE_NUMBER}</a></li>
                        <li className="flex gap-1 items-center text-xs"><Mail size={14} /> <a href={`mailto:${EMAIL_ADDRESS}`}>{EMAIL_ADDRESS}</a></li>
                    </span>
